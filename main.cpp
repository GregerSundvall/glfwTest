...
bool firstFrameMouseCorrection = true;
...
int main()
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // Load image, create texture and generate mipmaps
    data = stbi_load("Resources/Textures/smiley.png", &width, &height, &nrChannels, 0);
    if (data)
    {
        // Note that the smiley.png has transparency and thus an alpha channel, so make sure to tell OpenGL the data type is of GL_RGBA
...
void mouse_callback(GLFWwindow* window, double xPosIn, double yPosIn)
{
    ...
    if (firstFrameMouseCorrection)
    {
        lastX = xPos;
        lastY = yPos;
        firstFrameMouseCorrection = false;
    }
